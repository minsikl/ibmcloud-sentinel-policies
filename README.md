...
All policies are configured with `hard-mandatory` enforcement in [`sentinel.hcl`](sentinel-policies/sentinel.hcl).

## Usage with Terraform Enterprise

1. Connect the [`sentinel-policies`](sentinel-policies) directory as a VCS-based Sentinel policy set repository or policy set directory in Terraform Enterprise.
...
When adding or modifying a policy, register its `source` and `enforcement_level` in [`sentinel.hcl`](sentinel-policies/sentinel.hcl).
